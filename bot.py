...
def add_ball_conn(conn, uid, amount):
    """Mavjud connection orqali ball yangilash (lock bo'lmasligi uchun)."""
    conn.execute("UPDATE users SET ball = ball + ? WHERE user_id=?", (amount, uid))
...
# -----------------------------------------------------------------------
# 🌐 WEB API (Mini App uchun) — bot bazasi bilan to'g'ridan ulanish
# -----------------------------------------------------------------------
...
def validate_init_data(init_data_str):
    """Telegram WebApp initData ni tekshiradi va foydalanuvchi obyektini qaytaradi."""
    try:
...
def get_request_user():
    """initData ni tekshiradi, user_id qaytaradi yoki None."""
...
# ── BOOTSTRAP: barcha ma'lumotlarni bir so'rovda olish ──────────────────
@api.route('/api/data')
def api_data():
    ...
    conn = get_conn()
    urow = conn.execute("SELECT name,ball FROM users WHERE user_id=?", (uid,)).fetchone()
    ...
    nt = conn.execute(
        "SELECT bomdod,peshin,asr,shom,xufton,saved_at FROM namoz_times WHERE user_id=?", (uid,)).fetchone()
    namoz_times = None
    namoz_days_left = None
    if nt:
    ...
    today_namoz_rows = conn.execute(
        "SELECT namoz_nomi,holat FROM namoz_stats WHERE user_id=? AND sana=?", (uid, today_str())).fetchall()
    namoz_today = {}
    for r in today_namoz_rows:
        key = NAMOZ_KEY_MAP_REV.get(r['namoz_nomi'])
        if key:
            namoz_today[key] = NAMOZ_HOLAT_TO_APP.get(r['holat'], r['holat'])
    ...
    for h in habit_rows:
        log_today = conn.execute(
            "SELECT done FROM habit_logs WHERE habit_id=? AND sana=?", (h['id'], today_str())).fetchone()
    ...
    goal_rows = conn.execute(
        "SELECT id,title,deadline,reward,status,created,ball_reward FROM goals WHERE user_id=? ORDER BY id DESC",
        (uid,)).fetchall()
    goals = [dict(g) for g in goal_rows]

    reward_rows = conn.execute(
        "SELECT id,title,ball_cost,created FROM rewards WHERE user_id=? ORDER BY id DESC", (uid,)).fetchall()
    rewards = [dict(r) for r in reward_rows]

    conn.close()

    return jsonify({
        "user": {"id": uid, "name": name, "ball": ball},
        "unvon": unvon, "next_unvon": next_unvon, "next_ball": next_ball,
        "today": today_str(),
        "daily_tasks": [dict(t) for t in daily],
        "weekly_tasks": [dict(t) for t in weekly],
        "namoz_times": namoz_times,
        "namoz_days_left": namoz_days_left,
        "namoz_today": namoz_today,
        "namoz_week": namoz_week,
        "habits": habits,
        "goals": goals,
        "rewards": rewards,
    })
...
def api_weekly_add():
    ...
    conn = get_conn()
    conn.execute(
        "INSERT INTO weekly_tasks (user_id,task_name,task_time,category,priority) VALUES (?,?,?,?,?)",
...
def api_namoz_mark():
    ...
    body = request.get_json(force=True)
    key = body.get('namoz')
    holat_app = body.get('holat')  # 'oqildi' | 'endi' | 'qazo' | None
    if key not in NAMOZ_KEY_MAP:
        return jsonify({"error": "invalid"}), 400
    namoz_nomi = NAMOZ_KEY_MAP[key]
    conn = get_conn()
    existing = conn.execute(
        "SELECT id,holat FROM namoz_stats WHERE user_id=? AND namoz_nomi=? AND sana=?",
...
def api_habit_toggle():
    ...
    conn = get_conn()
    existing = conn.execute(
        "SELECT id,done FROM habit_logs WHERE habit_id=? AND user_id=? AND sana=?",
...
@api.route('/api/habit/delete', methods=['POST'])
def api_habit_delete():
    uid = get_request_user()
    if uid is None:
        return jsonify({"error": "unauthorized"}), 401
    body = request.get_json(force=True)
...
def api_goal_add():
    ...
    try:
        ...
        days = 0
    if not title or not reward or days <= 0:
        return jsonify({"error": "invalid"}), 400
    deadline = (uz_time() + timedelta(days=days)).strftime("%Y-%m-%d")
    conn = get_conn()
    conn.execute(
        "INSERT INTO goals (user_id,title,deadline,reward,created,ball_reward) VALUES (?,?,?,?,?,100)",
...
def api_goal_complete():
    ...
    gid = body.get('id')
    conn = get_conn()
    g = conn.execute("SELECT ball_reward,status FROM goals WHERE id=? AND user_id=?", (gid, uid)).fetchone()
    if not g or g['status'] == 'done':
        conn.close(); return jsonify({"error": "not_found"}), 404
    conn.execute("UPDATE goals SET status='done' WHERE id=?", (gid,))
    add_ball_conn(conn, uid, g['ball_reward'])
    conn.commit(); conn.close()
    return jsonify({"ok": True})

# ── MUKOFOTLAR ────────────────────────────────────────────────────────
@api.route('/api/reward/add', methods=['POST'])
def api_reward_add():
    uid = get_request_user()
    if uid is None:
        return jsonify({"error": "unauthorized"}), 401
...
def api_report():
    ...
    for ds in days:
        ...
        total_t += j; done_t += b
        nm = conn.execute(
            "SELECT COUNT(*) FROM namoz_stats WHERE user_id=? AND sana=? AND holat='oqildi'", (uid, ds)).fetchone()[0]
        namoz_t += nm
...
if __name__ == "__main__":
    ...
    # Web API server — Mini App va Render/UptimeRobot uchun
