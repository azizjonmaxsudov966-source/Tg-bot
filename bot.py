...
def cb_manage_avatar(call):
    ...
    except: bot.send_message(uid, text, reply_markup=markup, parse_mode="Markdown")
...
def cb_choose_style(call):
    ...
    except: bot.send_message(uid, "🎨 *Uslub tanlang:*", reply_markup=markup, parse_mode="Markdown")
    bot.answer_callback_query(call.id)
...
def cb_choose_frame(call):
    ...
    except: bot.send_message(uid, "🖼 *Ramka tanlang:*", reply_markup=markup, parse_mode="Markdown")
    bot.answer_callback_query(call.id)
...
def cb_choose_badge(call):
    ...
    except: bot.send_message(uid, "🏅 *Nishon tanlang:*", reply_markup=markup, parse_mode="Markdown")
    bot.answer_callback_query(call.id)
...
def cb_namoz_answer(call):
    ...
    except: bot.send_message(uid, text, parse_mode="Markdown")
...
def schedule_checker():
    ...
    while True:
        try:
            ...
            # Oylik hisobot (oy oxirgi kuni 21:30)
            ...
            # 9. JARIMA TIZIMI — kechki 23:30 bajarilmagan rejalar
            if current_time == "23:30":
                unfinished = conn.execute(
                    "SELECT user_id, COUNT(*) as cnt FROM daily_tasks "
                    "WHERE sana=? AND status IS NULL AND notified=1",
                    (today_str(),)).fetchall()
...
def api_goal_complete():
    ...
    if not g or g['status'] == 'done': conn.close(); return jsonify({"error": "not_found"}), 404
    conn.execute("UPDATE goals SET status='done' WHERE id=?", (gid,))
